    ...
    <div className="p-4 md:p-6 lg:p-8">
      ...
      <Tabs defaultValue="generator" className="mb-6">
        <div className="flex justify-between items-center mb-4">
          <TabsList className="bg-softBlue">
            <TabsTrigger value="generator" className="data-[state=active]:bg-skyBlue data-[state=active]:text-white">Image Generator</TabsTrigger>
            <TabsTrigger value="quotes" className="data-[state=active]:bg-vibrantPink data-[state=active]:text-white">Mindfulness Quotes</TabsTrigger>
          </TabsList>
          
          <div className="flex space-x-2">
            <button
              className={`p-2 rounded-md ${viewMode === 'grid' ? 'bg-goldenYellow text-white' : 'bg-softBlue text-darkGray'}`}
              onClick={() => setViewMode('grid')}
            >
              <Grid2X2 className="h-4 w-4" />
            </button>
            <button
              className={`p-2 rounded-md ${viewMode === 'list' ? 'bg-goldenYellow text-white' : 'bg-softBlue text-darkGray'}`}
        ...
        <TabsContent value="generator" className="mt-2">
          <Card className="cribbles-card">
            <CardHeader className="flex flex-row items-center pb-2 pt-6 px-6">
              <div className="w-6 h-6 rounded-full cribbles-gradient flex items-center justify-center">
                <MotionDot />
              </div>
              <CardTitle className="font-display font-semibold text-xl ml-2 text-darkGray">
            ...
            <CardContent className="p-6">
              <div className={viewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 gap-4' : 'space-y-4'}>
                {inspirationQuotes.map((item) => (
                  <Card key={item.id} className="bg-softBlue border-none shadow-sm">
                    <CardContent className="p-6">
                      <blockquote className="text-lg font-display mb-2 text-darkGray">
                        "{item.quote}"
                      </blockquote>
                      <footer className="text-right text-sm text-vibrantPink">
                        — {item.author}
                      </footer>
                    </CardContent>
                  </Card>
